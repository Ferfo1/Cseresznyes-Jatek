...
with open('Assets\Documentacion\ellenorzes.txt', 'r') as forrrasfajl:
    ...
    if ell == "freetrial":
        ...
        with open('Assets\Documentacion\score', 'r') as f:
            scorer = f.read()
        with open('Assets\Documentacion\data', 'r') as f:
            data = f.read()

        if data == "True":
            score = 100
            with open('Assets\Documentacion\data', 'w') as f:
                data = f.write("False")
        else:
            if messagebox.askyesno("?", f"Szeretnéd folytatni az előző játátékot?\nElőző játék egyenlege: {scorer}"):
                with open('Assets\Documentacion\score', 'r') as f:
                    score = int(f.read())
            else:
                score = 100
...
def exit():
    ...
    if score <= 10:
        ...
        with open('Assets\Documentacion\data', 'w') as f:
            f.write('True')
    with open('Assets\Documentacion\score', 'w') as f:
        f.write(str(score))
...
def tetelek():
    ...
    if getfut4 == True:
        ...
        tet.resizable(False, False)

    tet.attributes('-topmost', True)
    def disable_close():
        pass

    tet.protocol("WM_DELETE_WINDOW", disable_close)
    def t2():
    ...
    if getfut4 == True:
        ...
        et.pack()
        auto = Button(tet, text="Automatikus pörgetés", command=click)
        auto.pack()
...
def click_button(n):
    ...
    if n > 0:
        button16.invoke()
        root.after(3001, click_button, n-1)
...
def nyertel():
    ...
    nyv.resizable(False, False)

    nyv.attributes('-topmost', True)
    def disable_close():
        pass

    nyv.protocol("WM_DELETE_WINDOW", disable_close)
    font = ("Helvetica", 45)
...
def duplazas():
    ...
    global score, db, score_label, root
    db = Toplevel(root)
    db.title("Cseresznyés játék")
    ...
    db.protocol("WM_DELETE_WINDOW", disable_close)
    scoresave = 0
    scoresave = score
    choicecomp = "1", "2"
    ...
    def piros():
        ...
        if eredmeny == userchoice:
            bonuspont = bonuspont + bonuspont
            bonuspont = int(bonuspont)
            bonusm = f"{bonuspont}        "
            score_labelpont.config(text=f"Egyenleg: {bonusm}", font=150)
        else:
            bonuspont = 0
            ...
            score = int(score)
            scorem = f"{score}        "
            score_labelpont.config(text=f"Egyenleg: {scorem}", font=150)
    ...
    dbb1 = Button(db, text="Fekete", command=fekete, font=('Arial', 16))
    dbb1.pack()
    dbb2 = Button(db, text ="Piros", command=piros, font=('Arial', 16))
    dbb2.pack()
    dbb3 = Button(db, text="Felírás", command=feliras, font=('Arial', 16))
    dbb3.pack()
    db.mainloop()
...
def pontozas():
    ...
    pt.protocol("WM_DELETE_WINDOW", disable_close)    
...
def move_windows(event):
    """Update positions of child windows."""
    global prev_root_x, prev_root_y
    root_x, root_y = root.winfo_x(), root.winfo_y()
    for child in root.winfo_children():
        if isinstance(child, Toplevel):
            child_x, child_y = child.winfo_x(), child.winfo_y()
            child.geometry(f"+{root_x + child_x - prev_root_x}+{root_y + child_y - prev_root_y}")
    prev_root_x, prev_root_y = root_x, root_y

root.bind("<Configure>", move_windows)
...
score_labelpont = Label(text=f"Egyenleg: {scorem}", font=150)
